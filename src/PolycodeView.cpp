...
PolycodeView::PolycodeView(HINSTANCE hInstance, int nCmdShow, LPCTSTR windowTitle, 
	bool resizable, bool showDebugConsole) : PolycodeViewBase() {
	...
	wcex.lpszClassName	= "POLYCODEAPPLICATION";
	wcex.hIconSm		= LoadIcon(hInstance, IDI_APPLICATION);

	RegisterClassEx(&wcex);

	if(resizable) {
		hwnd = CreateWindowEx(WS_EX_APPWINDOW, "POLYCODEAPPLICATION", windowTitle,
			WS_OVERLAPPEDWINDOW|WS_SYSMENU, 0, 0, 640, 480, NULL, NULL, hInstance, NULL);
	} else {
		hwnd = CreateWindowEx(WS_EX_APPWINDOW, "POLYCODEAPPLICATION", windowTitle,
			WS_OVERLAPPED|WS_SYSMENU, 0, 0, 640, 480, NULL, NULL, hInstance, NULL);
	}
